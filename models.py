...
#--------- used in final model alongside resnet 3
...
class ResNet4(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1)) #added adaptive pool 1x1
        self.linear = nn.Linear(256*block.expansion, num_classes)
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        out = self.avgpool(out) #added
        # out = F.avg_pool2d(out, 4) #adaptive pool 4x4
        out = out.view(out.size(0), -1)
        out = self.linear(out)
        return out
...
#------Models for testing
#-----------Resnet 3 with Dropout rate modification
...
# Resnet 3 Layers: Feature maps (64, 128, 256) with averagpeooling 4by4
class ResNet3_4x4Avgpool(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        super(ResNet3_4x4Avgpool, self).__init__()
        self.in_planes = 64
        ...
        self.linear = nn.Linear(256*block.expansion*4*4, num_classes)
...
def Resnet3_4x4Avgpool():
    return ResNet3_4x4Avgpool(BasicBlock, [2, 2, 2])